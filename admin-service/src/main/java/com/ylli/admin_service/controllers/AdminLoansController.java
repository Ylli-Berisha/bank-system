...
public class AdminLoansController {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "loans retrieved successfully"), @ApiResponse(responseCode = "400", description = "Invalid filter parameters"), @ApiResponse(responseCode = "404", description = "No loans found matching the criteria"), @ApiResponse(responseCode = "500", description = "Internal server error while retrieving loans")})
    @GetMapping("/filter/loans")
    public ResponseEntity<Page<LoanDto>> getFilteredLoans(@RequestHeader("X-User-ID") String adminId, @RequestParam(required = false) String startDate, @RequestParam(required = false) String endDate, @RequestParam(required = false) String type, @RequestParam(required = false) String status, @RequestParam(required = false) BigDecimal minAmount, @RequestParam(required = false) BigDecimal maxAmount, @RequestParam(required = false) String userId, @RequestParam(required = false) String username, @RequestParam(required = false) String email, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "12") int size) {
        Page<LoanDto> loans = adminLoansService.getFilteredLoans(adminId, userId, username, email, type, status, startDate, endDate, minAmount, maxAmount, page, size);
    ...
    public ResponseEntity<LoanDto> acceptLoan(@RequestHeader("X-User-ID") String adminId, @PathVariable Long loanId) {
    ...
    public ResponseEntity<LoanDto> rejectLoan(@RequestHeader("X-User-ID") String adminId, @PathVariable Long loanId) {
        if (adminId == null || adminId.isEmpty() || loanId == null) {
            return ResponseEntity.badRequest().build();
        }
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Changes proposed successfully"), @ApiResponse(responseCode = "400", description = "Invalid input or loan cannot be modified"), @ApiResponse(responseCode = "404", description = "Loan not found"), @ApiResponse(responseCode = "403", description = "Admin validation failed")})
    @PutMapping("/{loanId}/propose-changes")
    public ResponseEntity<LoanDto> proposeChangesForLoan(@RequestHeader("X-User-ID") String adminId, @PathVariable Long loanId, @RequestBody LoanChangeProposalRequestDto proposalDto) {
